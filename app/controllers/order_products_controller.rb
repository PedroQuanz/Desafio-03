class OrderProductsController < ApplicationController
  ...
  def consulta
    @tabela_consulta1 = Person.all
    @tabela_consulta2 = Product.all
    @tabela_consulta3 = Order.find_by_sql("SELECT * FROM orders INNER JOIN people ON people.id = orders.person_id WHERE people.name = 'Jose'")
    @tabela_consulta4 = Order.find_by_sql("SELECT orders.description as compra, products.description, products.id, people.id, people.name FROM ((orders INNER JOIN people on people.id = orders.person_id)
			   INNER JOIN PRODUCTS ON PRODUCTS.ID = ORDERS.PRODUCT_ID) WHERE PRODUCTS.DESCRIPTION = 'Arroz'")
  end
